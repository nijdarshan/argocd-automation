...
| ArgoCD session expiry | Session tokens expire after ~24 hours (ArgoCD default `server.sessionMaxAge`). Not an issue in practice because the production design uses exit-and-resume — fresh token per batch run | Script re-authenticates before each operation. In production: exit-and-resume model means each batch gets a fresh token automatically |
...
This PoC validates the deployment flow. The production system documented in `../docs/06b-developer-requirements.md` has several design improvements that are NOT implemented in this code. The dev team should build to the 07b spec, not replicate this PoC.

| Area | This PoC | Production Design (07b) |
|------|----------|------------------------|
| **Execution model** | Long-running process — holds state in memory across all batches | **Exit-and-resume** — process exits after each batch, resumes from DB state on next trigger. Eliminates token expiry risk, survives crashes |
| **Token handling** | Single token at startup, reused for entire run | Fresh ArgoCD token per run — always valid because each batch is a new process |
| **API paths** | `/api/deploy`, `/api/rollback` | `/api/v1/deployments`, `/api/v1/rollbacks` — versioned, RESTful |
| **Auth** | None (lab environment) | JWT via Keycloak/SSO, RBAC roles (operator/viewer/admin), ArgoCD Cookie auth |
| **Rollback** | Single-step only (`git revert HEAD`) | Single-step (`git revert`) + multi-step (content-based `git show` restoration for targeting older deployments) |
| **Approval gates** | Blocks process until ENTER pressed (stdin) | Sets `pending_approval` in DB, process exits. `PUT /approve` triggers new run |
| **Deployment lock** | File-based (single process assumed) | DB-based with atomic SQL, TTL safety net (4h), stuck detection |
| **Database** | SQLite (single file) | MariaDB/PostgreSQL with HA |
| **Namespace management** | Namespaces pre-created in payload | Namespace manifest auto-updated when adding new components post-Day-0 |
| **Error responses** | Generic HTTP errors | Structured `{error: {code, message, details}}` with specific codes per failure type |
| **Notifications** | Console output only | Structured events (deployment.started, component.healthy, approval.required) — log-based initially, webhooks future |
| **TLS** | `-sk` (skip verification) | `--cacert` with proper CA bundle — never skip in production |
| **Git operations** | `git` subprocess calls | Same — `git` subprocess is the correct approach. No Git library needed |
| **ArgoCD API** | Same `curl`/`httpx` pattern | Same — Cookie auth, multi-source, resource-tree health. This is production-correct |
| **Per-component commits** | Same | Same — atomic commits per component, `git revert` for rollback |
| **Health checks** | Same resource-tree pattern | Same — pod health + service endpoints via ArgoCD API |

**What this PoC got right (carry forward to production):**
- Helm multi-source pattern (chart from Nexus + values from Git)
- Per-component atomic Git commits with standardised messages
- ArgoCD API-only cluster interaction (no kubectl at runtime)
- `force=false` for normal sync, `force=true` only for rollback
- Resource-tree health chain (Synced → Healthy → pods Ready → services have endpoints)
- App-of-apps with auto-sync+prune for child discovery, manual sync on children
- Batch sequencing from payload `deployment_order`
- State recording in DB (deployments + component_results + diffs)
...
|----------|----------|---------|
| Commands Reference | `../docs/06a-deployment-commands-reference.md` | Every ArgoCD API call and git command for each deployment operation |
